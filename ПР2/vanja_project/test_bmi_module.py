...
def test_calculate_bmi():
    # Проверяем правильный расчёт BMI
    assert round(calculate_bmi(70, 1.75), 1) == 22.9
    # Проверяем, что при нулевом росте вызывается ошибка
    with pytest.raises(ValueError):
        calculate_bmi(70, 0)

def test_bmi_category():
    # Проверяем категории BMI
...
def test_healthy_weight_range():
    # Проверяем диапазон здорового веса для роста 1.80 м
    min_w, max_w = healthy_weight_range(1.80)
    assert 59.9 <= min_w <= 60.0
    assert 80.5 <= max_w <= 81.0

def test_is_healthy_bmi():
    # Проверяем, что функция правильно определяет здоровый BMI
    assert is_healthy_bmi(22.5)
    assert not is_healthy_bmi(30)

def test_bmi_advice():
    # Эти тесты проверяют правильность советов по весу.
    # Они должны "поймать" намеренную ошибку в функции bmi_advice.
...
def test_bmi_advice_comprehensive():
    """Расширенные тесты для функции bmi_advice"""
    # Граничные значения
    ...
    # Крайние случаи
    assert bmi_advice(10) == "Слишком низкий вес"  # Сильный недовес
    assert bmi_advice(40) == "Слишком высокий вес" # Сильный перевес